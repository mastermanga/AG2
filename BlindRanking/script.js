...
function normalizeConfigObject(raw) {
  ...
  const mode =
    (modeParam === "songs" || modeParam === "song" || modeParam === "musique")
      ? "songs"
      : (modeParam === "anime" || modeParam === "animes")
      ? "anime"
      : null;
...
function getGlobalTopPctFromAnimeId(malId, fallbackMembers = 0) {
  ...
  while (lo <= hi) {
    const mid = (lo + hi) >> 1;
    if (GLOBAL_POP_SORTED[mid].members <= m) {
      ans = mid;
      hi = mid - 1;
    } else lo = mid + 1;
  }
...
fetch("../data/licenses_only.json")
  ...
  .then(json => {
    ...
    if (shouldAutoStart) {
      ...
      const urlRounds = parseInt(URL_PARAMS.get("count") || "", 10);
      const forcedRounds = Number.isFinite(urlRounds)
        ? urlRounds
