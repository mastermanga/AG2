 ...
 * - si ?parcours=1 :
 *   - masque "Menu"
 *   - masque la personnalisation
 *   - auto-start avec paramètres URL + fallback localStorage AG_parcours_filters
 *   - fin: bouton "Continuer le parcours" + postMessage parcoursScore
 *
 * ⚠️ Standard inchangé si pas de ?parcours=1
 **********************/
...
const PARAM_SONGS = urlParams.get("songs"); // ex: "OP,ED,IN" ou "opening,ending,insert"
...
// fallback config globale (comme Tournament)
...
/* Anti-flash parcours (script en bas => DOM déjà là) */
if (IS_PARCOURS) {
...
function showCustomization() {
  if (IS_PARCOURS) return; // ✅ en parcours on ne revient pas aux réglages
...
function initCustomUI() {
  ...
  applyBtn.addEventListener("click", () => {
    ...
    totalDuels = clampInt(parseInt(roundCountEl.value || "1", 10), 1, 100); // ✅ défaut = 1
...
function updatePreview() {
  ...
  const pool = applyFilters();
  const duelsWanted = clampInt(parseInt(roundCountEl.value || "1", 10), 1, 100); // ✅ défaut = 1
...
/* =======================
   PARCOURS: apply params
   ======================= */
function ensureDefaultTypesIfNone() {
...
function ensureDefaultSongsIfNone() {
  ...
  pills.forEach((p) => {
    const s = (p.dataset.song || "").toUpperCase();
    const on = s === "OP";
    p.classList.toggle("active", on);
    p.setAttribute("aria-pressed", on ? "true" : "false");
  });
...
function normalizeSongsParamToLorCodes(list) {
  // accepte: OP/ED/IN ou opening/ending/insert (case-insensitive)
  const out = [];
  for (const raw of list) {
    const s = String(raw || "").trim().toLowerCase();
    if (!s) continue;
    if (s === "op" || s === "opening" || s === "openings") out.push("OP");
    else if (s === "ed" || s === "ending" || s === "endings") out.push("ED");
    else if (s === "in" || s === "insert" || s === "inserts") out.push("IN");
  }
  return out;
}
...
function applyParcoursParamsToUI() {
  ...
  // mode (URL > cfg)
  const modeFromCfg = (cfg && (cfg.mode || cfg.lorMode)) || null;
  const wantedMode =
  ...
  if (songsListRaw && songsListRaw.length) {
    const songsList = normalizeSongsParamToLorCodes(songsListRaw);
    if (songsList.length) {
  ...
  // sécurité
  ensureDefaultTypesIfNone();
  ensureDefaultSongsIfNone();
  clampYearSliders();
...
fetch("../data/licenses_only.json")
  ...
  .then((json) => {
    ...
    if (IS_PARCOURS) {
      // ✅ applique params + auto-start
      applyParcoursParamsToUI();

      const pool = applyFilters();
      totalDuels = clampInt(parseInt(roundCountEl.value || "1", 10), 1, 100);
